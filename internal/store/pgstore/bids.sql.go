...
const listBidsByProductId = `-- name: ListBidsByProductId :many
SELECT id, product_id, bidder_id, bid_amount, created_at
FROM bids
WHERE product_id = $1
ORDER BY bid_amount DESC
...
func (q *Queries) ListBidsByProductId(ctx context.Context, productID uuid.UUID) ([]Bid, error) {
	rows, err := q.db.Query(ctx, listBidsByProductId, productID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
